...
def _moflix_api(monkeypatch, *, series=False):
    calls = []
    title = {"name": "Sample", "year": 2026, "poster": "/poster.jpg", "is_series": series}
    videos = [
    ...
    def fetch(url, *args, **kwargs):
        ...
        if "/api/v1/titles/42/seasons/1?" in url:
            return _response(payload={"episodes": {"data": [
        ...
        if url.endswith("/api/v1/titles/42"):
            return _response(payload={
                ...
                "seasons": {"data": [{"number": 1, "episodes_count": 2}]} if series else {},
            })
...
def test_movie_routes_include_only_implemented_hosters(client, monkeypatch):
    ...
    seasons = client.get("/api/seasons", query_string={"url": url}).get_json()["seasons"]
    assert len(seasons) == 1 and seasons[0]["are_movies"] is True
    episodes = client.get("/api/episodes", query_string={"url": url}).get_json()["episodes"]
    assert len(episodes) == 1 and episodes[0]["episode_number"] == 1
    providers = client.get("/api/providers", query_string={"url": url}).get_json()["providers"]
...
def test_series_listing_does_not_probe_every_episode(client, monkeypatch):
    ...
    assert providers.get_json()["providers"] == {"German Dub": ["Gupload", "MoflixClick"]}
...
def test_gupload_decodes_player_configuration(monkeypatch):
    ...
    encoded = base64.b64encode(bytes(value ^ key[i % len(key)] for i, value in enumerate(plain))).decode()
    html = (
    ...
    assert gupload.get_direct_link_from_gupload("https://gupload.xyz/e/sample") == stream
...
def test_vidara_uses_its_stream_api(monkeypatch):
    class Session:
        ...
        def post(self, url, *, json, headers, **kwargs):
            ...
            return _response(payload={"streaming_url": "https://cdn.example/master.m3u8"})
