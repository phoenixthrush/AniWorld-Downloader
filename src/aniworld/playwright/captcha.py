...
def _stealth_launch_args(offscreen: bool) -> list:
    ...
    if _sys.platform == "linux":
        # Force the X11/XWayland backend on Linux so Chromium doesn't attempt
        # native Wayland.  Under native Wayland, headed Chromium produces
        # incorrect iframe bounding boxes, causing click coordinates to miss
        # the Turnstile widget — and some Wayland compositors refuse to launch
        # a headed Chromium from a non-graphical process at all.
        # --ozone-platform=x11 is safe on pure-X11 desktops too (no-op).
